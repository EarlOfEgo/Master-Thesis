...
The aspect that every application which uses \textsc{BaseX} as a database solution has its own database, that stores, provides and updates the data.
...
The reason for this, as mentioned in Section~\ref{sec:android-internals}, is the sandbox principle of Android applications.
...
Thinking of different applications that are using the same data shows that this would lead to redundant databases, which would increase the required storage size.\\
...
Another improvement is that the files, that store the data, are on the server and not on the client.
Both advantages save the client storage space and execution resources, such as CPU and ram usage.
A disadvantage is that the client can only operate if there is a server available, and therefore it needs a connection to the server to send requests and receive responses.\\
With the migration of \textsc{BaseX} to Android, the client/server solution has also been migrated.
...
This has also been used for the Android solution of the client server implementation.
Additionally the server has been created as a service.
A service in Android is a process which is executed in the background and is thus the last instance which is killed by Android process management.
The reason to implement the server as an Android service is the process management of the operating system and the execution of it in the background.
The constraint of available RAM forces the Android to kill not needed application and free their used spaces in the RAM.
...
The Android operating system tries to keep the service as long as possible alive, depending on different criteria and even it is being killed, for example by the user, it tries to restart it.~\cite{android-service}
...
This application is also the sandbox where the server stores its data.
The \textsc{BaseX} uses,as well as the desktop version, the network for the communication with the clients.
Therefore the client application do not need to implement IPCs, they send their requests via the network to the server and also receive the responses through the network.
