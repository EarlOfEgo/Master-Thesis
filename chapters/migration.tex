...
In present chapter it is shown how the \textsc{BaseX} database has been migrated to the Android platform.
...
In Section~\ref{sec:migration:problems-during-the-migration} it is illustrated which parts of the \textsc{BaseX} version have been changed and how to receive a working \textsc{BaseX} Android version.
...
In the present section the two different platforms are analyzed.
...
Even if it is faster than other interpreted languages, it is still slower than into machine language compiled code which is executed directly on the hardware.
...
It also need to be mentioned that every operation which reads or writes to one of the above mentioned directories or files is an input output (IO) operation.
