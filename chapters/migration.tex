...
This folder is created by the database if it is not there yet.\\
Hence, \textsc{BaseX} has to figure out on which operating system it is running.
After \textsc{BaseX} knows the operating system it stores some operating system related values in a static class which is called \textit{Prop}, which need to be adjusted during the migration.
...
That \textsc{BaseX} can stores its databases inside the folder in the directory of the home directory of the current user it needs the absolute path to this folder.
Therefore the Java system method \textit{System.getProperty(``user.home'')} which returns the absolute path to the users directory on a normal JVM.\\
...
All classes inside this packages were only used by one \textsc{BaseX} package which is called org.basex.query.util.crypto.
...
Another possebility to support those functions would be using other cryptographic classes which are provided by Android.\\
...
As a result of this the depending classes of the GUI has also been removed, those are also not necessary because the \textsc{BaseX} library should not provide a graphical user interface.
...
Is it the first time the constructor of the class is used, it creates an directory which is called \textit{\textsc{BaseX}Data}.
...
The only way to get the application package name, except for hard coding, is to get the application context and extract the name there.
...
Another problem which occurred during the migration, was that \textsc{BaseX} uses a lot of the String function \textit{isEmpty()}, which is not available at an Android API-level lower than 9.
The same applies for the \textit{copyOf()} method in the \textit{java.util.Arrays} package.
This was solved by not supporting API-level lower than 9 with \textsc{BaseX} library.
...
To assure that no application which is build on Android version lower than 9 uses the \textsc{BaseX} library the Manifest file of the library has been adjusted.
This is one of the cases where the Manifest file of a library is used to determine the minimum Android version.
Other constraints like permissions are being ignored, like it was mentioned in Section~\ref{sec:migration:the-android-project-structure}.
